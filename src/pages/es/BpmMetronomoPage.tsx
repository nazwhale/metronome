...
export default function BpmMetronomoPage() {
  const { bpm: bpmParam } = useParams<"bpm">();
  const navigate = useNavigate();

  const bpm = useMemo(() => {
    if (!isValidBpmParam(bpmParam)) {
      navigate("/es/metronomo", { replace: true });
      return null;
    }
  ...
  const path = `/es/metronomo/${bpm}-bpm`;
  const hreflangUrls = {
  ...
  return (
    <Metronomo
      ...
      seoTitle={`Metrónomo ${bpm} BPM - Gratis Online | tempotick`}
      seoDescription={`Metrónomo online gratis a ${bpm} BPM. Tap tempo, compases, sin anuncios. Practica con un click estable de ${bpm} pulsaciones por minuto.`}
